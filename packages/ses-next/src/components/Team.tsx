      ...
      <div className="bg-base-200 py-6 sm:py-8 lg:py-12">
          ...
          <div className="flex flex-wrap items-stretch justify-center gap-4 lg:flex-row lg:gap-8">
            {members.map(({ avatar, fullName, role }) => (
              <div key={fullName} className="flex w-full flex-col items-center rounded-lg p-4 md:w-1/4 lg:p-8">
                <div className="bg-base-300 relative mb-2 h-24 w-24 overflow-hidden rounded-full shadow-lg md:mb-4 md:h-32 md:w-32">
                  <SanityImage
                    src={avatar}
                    alt={fullName}
                    fill
                ...
                <div>
                  ...
                  <p className="text-base-content/70 mb-3 text-center text-sm md:mb-4 md:text-base">{role}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 gap-6 rounded-lg p-6 sm:h-40 sm:content-evenly xl:grid-cols-4">
          {training.map(({ trainingTitle, icon }, idx) => (
            <div key={idx} className="text-base-content/70 flex items-center justify-center gap-1">

...
export function LocationNearbySuburbs({ suburb, nearbySuburbs }: LocationNearbySuburbsProps) {
  return (
    <section aria-labelledby="nearby-suburbs-heading" className="mx-auto mt-16 mb-12 max-w-5xl px-4 md:px-8">
      <div className="relative overflow-hidden rounded-xl border border-slate-200 bg-gradient-to-br from-slate-50 via-white to-primary/5 px-6 py-8 shadow-2xl shadow-slate-900/10 sm:px-8 lg:px-10">
        <div className="absolute inset-y-0 right-0 hidden w-1/3 bg-gradient-to-b from-primary/15 to-transparent lg:block" />
        ...
        <div className="relative grid gap-8 lg:grid-cols-2 lg:items-start">
          <div className="max-w-xl">
            <p className="mb-4 inline-flex rounded-full border border-primary/20 bg-white/80 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-primary shadow-sm">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {nearbySuburbs.map((location, index) => (
              <Link
                ...
                className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-lg shadow-slate-900/5 transition-all duration-200 hover:-translate-y-1 hover:border-primary/40 hover:shadow-xl hover:shadow-primary/10"
                prefetch={false}
              >

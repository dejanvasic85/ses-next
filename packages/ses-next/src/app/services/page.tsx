...
export async function generateMetadata(): Promise<Metadata> {
  const [{ siteSettings }, hubContent] = await Promise.all([
    getBasePageProps({ pageUrl: servicesPathValue }),
    getServicesHubContent(),
  ]);

  const title = `${hubContent.pageTitle ?? 'Services'} | ${siteSettings.companyName}`;
  const description = hubContent.pageDescription ?? siteSettings.meta.description;
...
export default async function ServicesPage() {
  const [{ services, siteSettings, pageUrl }, hubContent] = await Promise.all([
    getBasePageProps({ pageUrl: servicesPathValue }),
    getServicesHubContent(),
  ]);

  const topLevelServices = services.filter(({ parentService }) => !parentService);
      ...
      <div className="bg-white py-6 sm:py-8 lg:py-12">
        <div className="mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mx-auto mb-12 max-w-screen-xl text-center">
            {hubContent.heading && (
              <h1 className="mb-6 text-3xl font-bold text-gray-900 sm:text-4xl">{hubContent.heading}</h1>
            )}
            {hubContent.intro?.map((paragraph) => (
              <p key={paragraph} className="mb-3 text-lg text-gray-600">
                {paragraph}
              </p>
            ))}
          </div>
          ...
          <section aria-labelledby="cta-heading" className="mt-16 rounded-xl bg-primary/5 p-8 text-center">
